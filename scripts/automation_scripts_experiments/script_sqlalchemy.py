...
# Mata qualquer processo usando a porta UDP 50001 (ignora erro se não houver nenhum)
    ...
    for i in range(15, num_experiments + 1):
        src_instance = base_dir / f"sqlalchemy_{i}"
        working_dir = base_dir / "sqlalchemy"
        completed_dir = base_dir / f"sqlalchemy_{i}_done"

        print("#################################################################")
        print(f"\n🔁 Iniciando experimento {i}: sqlalchemy_{i}")
        print("#################################################################")
...
def activate_virtualenv(command: str):
    try:
        ...
        full_cmd = (
                f'bash -c "'
                f'cd \\"/media/pedrinho/MESTRADO/DISSERTACAO/Experimento_Final/project/sqlalchemy\\" && '
...
def run_tests(experiment_number):
    ...
    try:
        ...
        if failures < 5:
            ...
            run_sonarqube(f"sqlalchemy_{experiment_number}", Path(project_path), venv_name)
            return True
        else:
            generate_logs(log_output + "\n" + log_stderr)
...
def classify_sonarqube_issues():
    ...
    with open(classification_output_path, "a") as f_out:
        ...
        f_out.write("High: High | Medium: Medium | Low: Low\n")
