...
# Mata qualquer processo usando a porta UDP 50001 (ignora erro se não houver nenhum)
    ...
    for i in range(11, num_experiments + 1):
        src_instance = base_dir / f"mitmproxy_{i}"
        working_dir = base_dir / "mitmproxy"
        completed_dir = base_dir / f"mitmproxy_{i}_done"

        print("#################################################################")
        print(f"\n🔁 Iniciando experimento {i}: mitmproxy_{i}")
        print("#################################################################")
...
def activate_virtualenv(command: str):
    try:
        ...
        full_cmd = (
                f'bash -c "'
                f'cd \\"/media/pedrinho/MESTRADO/DISSERTACAO/Experimento_Final/project/mitmproxy\\" && '
...
def count_real_failures(log_output: str) -> int:
    ...
    error_keywords = [
        ...
        "ERROR",
        "NameError",        # Erros comuns de nome
        "AttributeError",    # Erros comuns de atributo
        "TypeError",         # Erros comuns de tipo
        "AssertionError",    # Erros de asserção
        "KeyError",          # Erros comuns de chave
        "IndexError",        # Erros comuns de índice
        "ValueError",        # Erros comuns de valor
        "ZeroDivisionError", # Erros comuns de divisão por zero
        "FileNotFoundError", # Erros comuns de arquivo não encontrado
        "IOError",           # Erros de entrada/saída
        "OSError",           # Erros do sistema operacional
        "RuntimeError",      # Erros de tempo de execução
        "ImportWarning",     # Avisos de importação                                                   # Testes com erro (ex: setup falhou)
...
def run_tests(experiment_number):
    ...
    try:
        ...
        if failures == 1:
            ...
            run_sonarqube(f"mitmproxy_{experiment_number}", Path(project_path), venv_name)
            return True
        else:
            generate_logs(log_output + "\n" + log_stderr)
