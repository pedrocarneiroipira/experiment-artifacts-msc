...
# Mata qualquer processo usando a porta UDP 50001 (ignora erro se não houver nenhum)
    ...
    for i in range(13, num_experiments + 1):
        src_instance = base_dir / f"pandas_{i}"
        working_dir = base_dir / "pandas"
        completed_dir = base_dir / f"pandas_{i}_done"

        print("#################################################################")
        print(f"\n🔁 Iniciando experimento {i}: pandas_{i}")
        print("#################################################################")
...
def activate_virtualenv(command: str):
    try:
        ...
        full_cmd = (
                f'bash -c "'
                f'cd \\"/media/pedrinho/MESTRADO/DISSERTACAO/Experimento_Final/project/pandas\\" && '
...
def run_tests(experiment_number):
    ...
    try:
        ...
        match experiment_number:
            case 1:
                output = "pytest -vv pandas/tests/arrays/sparse/test_array.py "
            case 2:
                output = "pytest -vv pandas/tests/series/methods/test_update.py "
            case 3:
                output = "pytest -vv pandas/tests/frame/methods/test_asof.py "
            case 4: # Testes com pytest-xdist
                output = "pytest -vv pandas/tests/io/formats/test_printing.py "
            case 5: # Testes com pytest-xdist
                output = "pytest -vv pandas/tests/series/methods/test_isin.py "
            case 6:
                output = "pytest -vv pandas/tests/series/methods/test_isin.py "                
            case 7:
                output = "pytest -vv pandas/tests/reshape/test_pivot.py "
            case 8:
                output = "pytest -vv pandas/tests/arrays/boolean/test_logical.py "
            case 9:
                output = "pytest -vv pandas/tests/io/formats/test_to_string.py "          
            case 10:
                output = "pytest -vv pandas/tests/io/formats/test_to_html.py "
            case 11:
                output = "pytest -vv pandas/tests/io/formats/test_to_string.py "
            case 12:
                output = "pytest -vv pandas/tests/indexes/multi/test_sorting.py "
            case 13:
                output = "pytest -vv pandas/tests/test_sorting.py "
            case 14:
                output = "pytest -vv pandas/tests/io/excel/test_readers.py "
            case 15:
                output = "pytest -vv pandas/tests/tseries/holiday/test_holiday.py "

        log_output, log_stderr = activate_virtualenv(output)

        fatal_keywords = [
            "importerror", "modulenotfounderror", "syntaxerror", "notimplementederror",
        ...
        if failures <= 5:
            ...
            run_sonarqube(f"pandas_{experiment_number}", Path(project_path), venv_name)
            return True
        else:
            generate_logs(log_output + "\n" + log_stderr)
