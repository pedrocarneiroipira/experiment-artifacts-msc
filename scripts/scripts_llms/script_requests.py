...
# Mata qualquer processo usando a porta UDP 50001 (ignora erro se não houver nenhum)
    ...
    for i in range(3, num_experiments + 1):
        src_instance = base_dir / f"requests_{i}"
        working_dir = base_dir / "requests"
        completed_dir = base_dir / f"requests_{i}_done"
...
def activate_virtualenv(command: str):
    try:
        ...
        full_cmd = (
                f'bash -c "'
                f'cd \\"/media/pedrinho/MESTRADO/DISSERTACAO/Experimento_Final/project/requests\\" && '
...
def run_tests(experiment_number):
    ...
    try:
        ...
        if failures == 0:
            ...
            run_sonarqube(f"requests_{experiment_number}", Path(project_path), venv_name)
            return True
        else:
            generate_logs(log_output + "\n" + log_stderr)
