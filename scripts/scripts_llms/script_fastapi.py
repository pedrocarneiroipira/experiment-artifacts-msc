...
# Mata qualquer processo usando a porta UDP 50001 (ignora erro se não houver nenhum)
    ...
    for i in range(1, num_experiments + 1):
        src_instance = base_dir / f"djangorest_{i}"
        working_dir = base_dir / "djangorest"
        completed_dir = base_dir / f"djangorest_{i}_done"

        print("#################################################################")
        print(f"\n🔁 Iniciando experimento {i}: djangorest_{i}")
        print("#################################################################")
...
def activate_virtualenv(command: str):
    try:
        ...
        full_cmd = (
                f'bash -c "'
                f'cd \\"/media/pedrinho/MESTRADO/DISSERTACAO/Experimento_Final/project/djangorest\\" && '
...
def run_tests(experiment_number):
    ...
    try:
        ...
        if failures == 1:
            ...
            run_sonarqube(f"djangorest_{experiment_number}", Path(project_path), venv_name)
            return True
        else:
            generate_logs(log_output + "\n" + log_stderr)
